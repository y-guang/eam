...
boundary), and we add a `condition_idx` field to indicate that all
...
(`method = "loclinear"`), which learns a flexible mapping from summary
statistics to parameters to improve posterior approximation.
...
The `resid_tol` option trims extreme residuals (here retaining the
central 99%), making the main recovery pattern easier to inspect.
...
Model comparison is conducted using the
...
then aligns simulated data and parameters into a consistent format for
training.
...
The input dimension `d = 2` corresponds to the two observed variables
per trial (choice and RT). The DeepSet encoder maps a set of trials into
a fixed-length representation, which is then transformed into parameter
estimates.

The output dimension equals the number of parameters (`p = 3`), with
each parameter predicted by a dedicated output unit using a softplus
...
After training, we obtain point estimates of the parameters for the
observed dataset.
...
performs a validation procedure by:

1.  sampling datasets with known parameters
2.  re-estimating parameters using the trained estimator
3.  comparing estimated vs true parameters

Good recovery is indicated by high correlation between estimated and
...
We assess model fit by generating simulated data using the estimated
parameters and comparing them to the observed data.

This allows us to evaluate whether the model reproduces RT distributions
...
Instead of predicting a single point estimate, we learn the full
posterior distribution.

The input dimension `d = 2` corresponds to the two observed variables
per trial (choice and RT).

The DeepSet encoder maps a set of trials into a fixed-length
representation.

The output dimension equals the number of parameters (`p = 3`).

Instead of producing point estimates, a normalizing flow is used to
...
#> 3          1     ndt_0 0.5222758 0.5601937     0.02438583      0.9521827
```
...
We evaluate posterior recovery by comparing median of posteiror samples
...
We assess model fit by generating simulated data using the median of
posteiror samples of parameters and comparing them to the observed data.
