...
#> Warning in file(file, "rt"): cannot open file 'example_data.csv': No
...
#> Error in `dplyr::select()` at eam/R/plot_rt.R:60:3:
#> ℹ In argument: `dplyr::all_of(cols_to_select)`.
#> Caused by error in `dplyr::all_of()`:
#> ! Can't subset elements that don't exist.
#> ✖ Elements `item_idx` and `rank_idx` don't exist.
```
