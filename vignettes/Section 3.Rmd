...
Here, we used the free recall data from Penn Electrophysiology of Encoding and Retrieval Study (PEERS; Kahana et al., 2024).
We model the recall process using a multi-response Racing Diffusion Model (RDM), focusing on RTs for first seven retrievaled items. 
...
structural aspects of the task. Specifically, the decision boundary is assumed to vary with the \emph{output position}, whereas the drift rate varies with the \emph{item index}. 
...
\end{aligned}
\]
where \( \varepsilon > 0 \) is a small constant ensuring positivity of the drift rate.
...
```
When parallelization is enabled, running the full simulation
typically takes approximately 30 minutes to 1 hour.
Given the computational constraints of this tutorial, we do not execute the full
pipeline here. Interested readers are encouraged to run the code locally to
...
```
For illustration purposes, we use data from one representative participant in the
PEERS dataset as the observed data in this example.
```{r}
#######################
# Load Observed data  #
#######################

observed_data<-read.csv("example_data.csv")
```
Here we calcualte the response time quantiles (0.1–0.9) separately for each response
position ("rank_idx").
```{r,eval=FALSE}
#####################
# abc model prepare #
...
As the simulation step is omitted here, the inference code below is presented
without being executed.
Readers can run the complete inference pipeline locally once the simulated data have been
generated.
...
```
Here, we directly present results from a previously fitted model and assess
model adequacy by examining the overlap between RTs simulated from the
posterior median and the RTs.
```{r}
##############################
